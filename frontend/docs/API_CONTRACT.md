...
- `POST /v1/quotation-requests` - creates a `SUBMITTED` RFQ with customer, free-text KBI SAP PO ref, optional KBI `customer_contract_ref`, supplier, incoterm, mode, currency, POL/POD, desired cargo-ready date, cargo hints, `lines[]`, `packages[]`/`containers[]`, and note.
  ...
  - `packages[]` (mode `SEA_LCL`/`AIR`) items: `{ package_no, package_type, length_cm, width_cm, height_cm, qty, gross_weight_per_package_kg, cbm, lines, note, parent_package_no }`. `package_type` is free text (e.g. "Carton", "Pallet") - not a fixed enum - since packers describe cargo units in their own wording. Frontend derives each package `cbm = qty * length_cm * width_cm * height_cm / 1_000_000`; `qty` is the physical package count (e.g. number of cartons on a pallet) and is independent of any item quantity. `lines[]` is the nested list of commercial line items packed inside that physical unit - one package can hold several different SKUs: `{ line_no, item_id, item_description, qty, unit, unit_price, note }` (mirrors `containers[].lines[]`, minus per-line weight since gross weight is captured once at the package level via `gross_weight_per_package_kg`). `parent_package_no` (nullable) supports nested packaging - e.g. Cartons loaded onto a Pallet - by referencing another package in the same `packages[]` array; `volume_cbm` on the header is summed only over packages with no `parent_package_no` (top-level units) to avoid double-counting a carton's volume that is already inside its parent pallet, while `gross_weight_kg` sums every package's own `qty * gross_weight_per_package_kg` (a parent's manually entered gross plus its nested children's gross all roll up into the header total).
...
- `GET|POST /v1/quotations/:id/charge-lines` - charge-line DTOs carry per-line `currency_code` and `charge_group` (`FREIGHT|ORIGIN|DESTINATION`). **`line.currency_code` is the source/local fee currency; `quotation.currency_code` is the customer payment currency** — there is no persisted per-line settlement/endpoint currency. The quotation form stores charges in three manual groups; each line still renders in its own source currency for audit, but every customer-facing total (Create rail, option rows, Detail hero, Charge Breakdown) converts line amounts through `/v1/currency-rates` into `quotation.currency_code` and reports one total, not per-currency subtotals. Each charge-line DTO also carries `option_no` (`number | null`): `null` = shared across all options (ORIGIN/DESTINATION/local); a value `N` links the line to quotation option `option_no = N` (per-option FREIGHT). The quotation detail shows shared lines plus the selected option's freight.
