...
export function RfqQuickSummaryStrip({
  ...
  return (
    <Paper withBorder p={0} className="rfq-detail-summary-strip">
      <SummaryItem
      ...
      <SummaryItem
        ...
        value={buildRfqRouteLabel(request)}
        meta={[request.mode, request.incoterm_code, request.currency_code].filter(Boolean).join(' / ') || '-'}
      />
      <SummaryItem
