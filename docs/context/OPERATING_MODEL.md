...
2. DO (Delivery Order) — Incorporates Quotations
   Each LOT creates 1 DO. DO defines origin warehouse, destination warehouse,
   transport type, and delivery schedule.
   * Quotation (Sub-flow): FDS Sales creates freight-forwarding quotation
     under the DO. KBI reviews, rejects/revises, or approves.

3. Shipment — Incorporates Documents & 10 Milestones
   Logistics consolidates one or more DO lines into a Shipment.
...
| Logistics | Create shipment from DOs, maintain forwarder/carrier, update milestones, manage B/L or AWB, manage delivery schedule. |
...
| State | Entry condition | Allowed next states |
|---|---|---|
| `DRAFT` | DO auto-created from PO LOT | `CONFIRMED`, `CANCELLED` |
| `CONFIRMED` | Buyer/Logistics confirms DO details (origin, destination, transport type) | `READY_TO_SHIP`, `CANCELLED` |
...
DO required fields:
- Origin warehouse (kho đi)
- Destination warehouse (kho đến)
...
| State | Entry condition | Allowed next states |
|---|---|---|
| `DRAFT` | FDS Sales creates new quotation or revises after rejection | `SENT`, `CANCELLED` |
...
|---|---|
| PO LOT default | A new PO defaults to 1 LOT containing all items, which creates 1 DO. |
| LOT split | Splitting a LOT creates additional DOs. Items can be dragged between LOTs (no-code). |
| DO confirmation | A DO cannot proceed to `READY_TO_SHIP` without origin warehouse, destination warehouse, transport type, and explicit confirmation. |
...
| Quotation versioning | Rejecting a quotation returns it to DRAFT. The same version can be revised. A new version number is assigned only for substantial changes. |
...
| DO | Origin warehouse → Destination warehouse → Transport type → Delivery dates → Confirm |
| Quotation (in DO) | Reference DO/Shipment → Service type → Pricing details → Terms → Attachments → Submit |
| Shipment | Mode → Forwarder → Carrier → B/L/AWB → Route (POL/POD) → Dates → DO lines |
