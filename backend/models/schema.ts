...
import { seedStandardTemplates } from './poStageTasks';
import { seedStandardApprovalMatrix } from './approval';

function migrationUrl(fileName: string) {
  const builtUrl = new URL(`./migrations/${fileName}`, import.meta.url);
  return existsSync(builtUrl) ? builtUrl : new URL(`../migrations/${fileName}`, import.meta.url);
}
...
export async function ensureSchemaAndSeed() {
  const migrationSql = await readFile(migrationUrl('001_normalized_logistics_schema.sql'), 'utf8');
  ...
  const migration002Sql = await readFile(migrationUrl('002_gd1_core_tables.sql'), 'utf8');
  await pool.query(migration002Sql);

  const migration003Sql = await readFile(migrationUrl('003_gd1_field_additions.sql'), 'utf8');
  await pool.query(migration003Sql);

  const migration004Sql = await readFile(migrationUrl('004_reliability_integration_foundation.sql'), 'utf8');
