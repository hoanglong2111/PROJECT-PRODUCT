...
const repoRoot = fileURLToPath(new URL('.', packageJsonUrl));
const migrationUrls = [
  ...
  new URL('../migrations/005_user_preferences.sql', import.meta.url),
];
const protocolVersion = '2024-11-05';

const requiredRuntimeEnv = ['CORS_ORIGIN', 'DATABASE_URL', 'JWT_SECRET'];
...
async function checkHealth(apiBaseUrl?: string) {
  const baseUrl = apiBaseUrl ?? process.env.API_BASE_URL ?? 'http://localhost:4000/api';
  ...
  if (uri === 'kbfe://risk-queue') {
    return {
      risks: [
        ...
        'Frontend and backend deploy independently; keep API_BASE_URL aligned with the deployed backend.',
      ],
    };
  }
  ...
  return [
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          apiBaseUrl: { type: 'string', description: 'Optional API base URL. Defaults to API_BASE_URL.' },
        },
